...
describe("S85-C1 SGK katalog hazirlik parity", () => {
  ...
  it("wires router, endpoints, UI panel and docs without seed/039", () => {
    ...
    expect(router).toContain("/sgk-katalog-hazirlik/import");
    ...
    expect(migrationNames.filter((n) => n.endsWith(".sql")).sort().at(-1)).toBe("059_retention_physical_destruction_execution.sql");
