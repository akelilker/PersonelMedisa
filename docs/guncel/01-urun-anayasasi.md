...
- S75-B/C puantaj çakışma çözümü (`/cakisma-coz`) yalnız `MUHASEBE` (`puantaj.bildirim_etki.resolve_conflict`) için lokal kod paketinde tamamlandı: mevcut `gunluk_puantaj` satırına karşı **Mevcut Puantajı Koru** (`YOK_SAYILDI`) veya **Aday Etkisiyle Revize Et** (`UYGULANDI`, aynı satır UPDATE); karar audit tablosu `bildirim_puantaj_etki_cakisma_cozumleri` (migration `015`); yeni aday state'i yok. Canlı deploy/push yok; final etiket: `S75_BC_CONFLICT_RESOLUTION_LOCAL_COMPLETE`.
