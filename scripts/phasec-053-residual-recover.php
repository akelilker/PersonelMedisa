<?php
/**
 * Phase C 053 residual recovery helper (OPS ONLY â€” not product code).
...
if ($action === 'residual') {
    ...
    // Snapshot cols should exist via CREATE â€” verify, do not ALTER
    ...
    respond([
        ...
        'identity' => $id,
        'ops' => $ops,
        'applied' => count(array_filter($ops, function ($o) { return ($o['result'] ?? '') === 'APPLIED'; })),
...
if ($action === 'postcheck') {
    ...
    $ok = !in_array(false, $presence, true)
        ...
        && $snapOk
        && $roleOk
        && !in_array(false, array_map(function ($c) { return $c === 0; }, $counts), true);
    respond([
        'ok' => $ok,
        'table_presence' => $presence,
        'manifest' => $manifest,
