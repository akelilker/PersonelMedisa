...
Şenay Mobilya; target runtime enum `AY_1_SON_GUN` for branches `1,4,5,6,7,8,9,10,11`;
released canonical readback matched branch `1`, while branches `4,5,6,7,8,9,10,11`
returned `NO_APPROVED_POLICY`; production rollout is `OPS_ROLLOUT_ACTIVE` pending
canonical draft → submit → separate approval.
...
| SGK reporting period | **CLOSED_CONFIRMED** business decision; target `AY_1_SON_GUN`; released readback: branch `1` matched, branches `4,5,6,7,8,9,10,11` returned `NO_APPROVED_POLICY`; production rollout `OPS_ROLLOUT_ACTIVE`; `15_TO_NEXT_MONTH_14` and `MIXED_BY_INSURED` not used |
