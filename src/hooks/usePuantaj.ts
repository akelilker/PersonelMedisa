...
export function usePuantaj() {
  ...
  const gunlukEksikGunSiniflandirmasi = useMemo((): PuantajEksikGunSiniflandirmaSonucu | null => {
    if (!activeQuery || !puantaj) {
      return null;
    }

    void appDataRevision;
    const hastalikRaporSurecleri = readHastalikRaporSurecleriFromCache(
      activeSube,
      activeQuery.personelId
    );
    return siniflandirGunlukPuantajEksikGunEtkisi(puantaj, hastalikRaporSurecleri);
  }, [activeQuery, activeSube, puantaj, appDataRevision]);
