<?php
/**
 * ONE-SHOT S79-F-R3 live migrate for 031_haftalik_kapanis_revizyon_corrections.sql.
...
$tokenExpected = 'REPLACE_S79FR3_MIGRATE_TOKEN';
$tokenProvided = isset($_GET['token']) ? (string) $_GET['token'] : '';
// Workflow greps for UNSET_S79FR3_MIGRATE_TOKEN; live token must be openssl rand -hex 24.
if (
...
const S79_EXPECTED_MIGRATION_SHA256 = '83ec409a8390bae5c3cfcc04617a65e30de99ef5a3736d77b71d211a537a8599';
const S79_MIGRATION_FILE = '031_haftalik_kapanis_revizyon_corrections.sql';
const S79_SMOKE_MARKER = 'S79-F-R3 Production Smoke';
...
const S79_SMOKE_TC = '90079000039';
const S79_SMOKE_SICIL = 'S79FR3-SMOKE';
...
{
    $out = [];
    $out[] = '-- S79-F-R3 PHP SQL dump (shared-host fallback)';
    $out[] = '-- Database: ' . $dbName;
...
{
    return __DIR__ . '/s79fr3_smoke_marker.json';
}
...
if ($action === 'preflight') {
    ...
    if ($identity['aktif_veritabani'] !== 'karmotor_medisa') {
        ...
        echo json_encode([
            'ok' => false,
            'code' => 'S79_F_R3_BLOCKED_DB_IDENTITY',
            'identity' => $identity,
        ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
    ...
    if (!$parentOk) {
        $code = 'S79_F_R3_BLOCKED_PARENT_SCHEMA';
        $ok = false;
    } elseif ($existing['partial']) {
        $code = 'S79_F_R3_BLOCKED_EXISTING_SCHEMA_DRIFT';
        $ok = false;
    } elseif ($existing['both_present']) {
        $schemaMatch = s79_schema_matches_contract($pdo);
        if ($schemaMatch['ok']) {
            $code = 'S79_F_SCHEMA_ALREADY_APPLIED';
            $ok = true;
        } else {
            $code = 'S79_F_R3_BLOCKED_EXISTING_SCHEMA_DRIFT';
            $ok = false;
    ...
    echo json_encode([
        'ok' => $ok,
        'code' => $code,
        'already_applied' => $existing['both_present'] && $ok && $code === 'S79_F_SCHEMA_ALREADY_APPLIED',
        'identity' => $identity,
        'parents' => $parents,
        'existing' => $existing,
        'counts' => $counts,
...
if ($action === 'backup') {
    ...
    if ($identity['aktif_veritabani'] !== 'karmotor_medisa') {
        http_response_code(409);
        echo json_encode(['ok' => false, 'code' => 'S79_F_R3_BLOCKED_DB_IDENTITY', 'identity' => $identity], JSON_UNESCAPED_UNICODE);
        exit;
    }
    ...
    file_put_contents(__DIR__ . '/s79fr3_latest_backup_path.txt', basename($backupPath));

    echo json_encode([
        'ok' => $ok,
        'code' => $ok ? 'S79_F_BACKUP_OK' : 'S79_F_R3_BLOCKED_BACKUP',
        'backup' => $meta,
        'identity' => $identity,
        'counts' => s79_counts($pdo),
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
...
if ($action === 'download_backup') {
    $marker = __DIR__ . '/s79fr3_latest_backup_path.txt';
    $backupPath = '';
    if (is_file($marker)) {
...
if ($action === 'migrate') {
    ...
    if ($existing['both_present']) {
        ...
        echo json_encode([
            'ok' => $match['ok'],
            'code' => $match['ok'] ? 'S79_F_MIGRATE_OK' : 'S79_F_R3_BLOCKED_EXISTING_SCHEMA_DRIFT',
            'skipped_apply' => true,
    ...
    if (!hash_equals(S79_EXPECTED_MIGRATION_SHA256, strtolower($sha))) {
        ...
        echo json_encode([
            'ok' => false,
            'code' => 'S79_F_R3_BLOCKED_MIGRATION_DRIFT',
    ...
    try {
        ...
        echo json_encode([
            'ok' => false,
            'code' => 'S79_F_R3_BLOCKED_MIGRATION_APPLY',
            'error' => $e->getMessage(),
    ...
    echo json_encode([
        'ok' => $ok,
        'code' => $ok ? 'S79_F_MIGRATE_OK' : 'S79_F_R3_BLOCKED_MIGRATION_APPLY',
        'skipped_apply' => false,
...
if ($action === 'postcheck') {
    ...
    echo json_encode([
        'ok' => $ok,
        'code' => $ok ? 'S79_F_POSTCHECK_OK' : 'S79_F_MIGRATION_APPLIED_SCHEMA_ACCEPTANCE_BLOCKED',
        'identity' => $identity,
        'counts' => $counts,
        'schema_match' => $match,
    ], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
...
if ($action === 'integrity') {
    ...
    echo json_encode([
        'ok' => $ok,
        'code' => $ok ? 'S79_F_FINAL_INTEGRITY_OK' : 'S79_F_INTEGRITY_FAILED',
        'counts' => $counts,
...
if ($action === 'smoke_prepare') {
    ...
    if ($identity['aktif_veritabani'] !== 'karmotor_medisa') {
        http_response_code(409);
        echo json_encode(['ok' => false, 'code' => 'S79_F_R3_BLOCKED_DB_IDENTITY'], JSON_UNESCAPED_UNICODE);
        exit;
    }
    if (!s79_table_exists($pdo, 'haftalik_kapanis_revizyon_talepleri')
    ...
    try {
        ...
        $insP = $pdo->prepare(
            "INSERT INTO personeller (
                ...
                :tc, 'S79FR3', 'Smoke', '1990-01-01', '05000000000', 'S79FR3 Contact', '05000000001',
        ...
        for ($i = 0; $i < 5; $i++) {
            $insSurec->execute([
                ...
                'bas' => $etkilenenTarih,
                'bit' => $etkilenenTarih,
                'aciklama' => S79_SMOKE_MARKER . ' closed-' . $i,
            ]);
        ...
        http_response_code(500);
        echo json_encode(['ok' => false, 'code' => 'S79_F_SMOKE_PREPARE_FAILED', 'error' => $e->getMessage()], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
...
if ($action === 'smoke_run') {
    ...
    $mainOnay = s79_api_create_onaylandi(
        $meta,
        s79_rt_create_body($personelId, $snapshotId, $etkilenen, $hb, $he, S79_SMOKE_MARKER . ' main'),
        $baH,
        $gyH
    );
    ...
    foreach ($stateDefs as $def) {
        ...
        $body = s79_rt_create_body(
            ...
            $hb,
            $he,
            S79_SMOKE_MARKER . ' state-' . $def['name'],
            [
                'kaynak_tipi' => 'SUREC',
        ...
        if ($def['leave'] === 'REDDEDILDI') {
            s79_http('POST', '/haftalik-kapanis/revizyon-talepleri/' . $tid . '/red', ['karar_notu' => S79_SMOKE_MARKER . ' red'], $gyH);
        }
        if ($def['leave'] === 'IPTAL') {
    ...
    $cancelOk = s79_http(
        'POST',
        '/haftalik-kapanis/revizyon-corrections/' . $correctionId . '/iptal',
        ['aciklama' => S79_SMOKE_MARKER . ' cancel'],
        $gyH
    );
    $pass(
        'cancel success',
    ...
    $concTalep = s79_api_create_onaylandi(
        ...
        s79_rt_create_body(
            ...
            $hb,
            $he,
            S79_SMOKE_MARKER . ' concurrency-prod',
            ['bordro_etki_var_mi' => false]
        ),
        $baH,
        $gyH
    ...
    echo json_encode([
        'ok' => $ok,
        'code' => $ok ? 'S79_F_SMOKE_RUN_OK' : 'S79_F_SMOKE_RUN_FAILED',
        'steps' => $steps,
        'fixture' => $fixturePublic,
...
if ($action === 'smoke_cleanup') {
    ...
    try {
        ...
        if ($talepIds !== [] && s79_table_exists($pdo, 'haftalik_kapanis_revizyon_talepleri')) {
            ...
            foreach ($verify->fetchAll() ?: [] as $row) {
                if (strpos((string) ($row['gerekce'] ?? ''), S79_SMOKE_MARKER) !== false) {
                    $safeIds[] = (int) $row['id'];
                }
            }
            if ($safeIds !== []) {
        ...
        http_response_code(500);
        echo json_encode(['ok' => false, 'code' => 'S79_F_SMOKE_CLEANUP_FAILED', 'error' => $e->getMessage(), 'deleted' => $deleted], JSON_UNESCAPED_UNICODE | JSON_UNESCAPED_SLASHES);
        exit;
    }
    ...
    echo json_encode([
        'ok' => $ok,
        'code' => $ok ? 'S79_F_SMOKE_CLEANUP_OK' : 'S79_F_SMOKE_CLEANUP_INCOMPLETE',
        'deleted' => $deleted,
        'marker_rows' => $markers,
        'counts' => s79_counts($pdo),
