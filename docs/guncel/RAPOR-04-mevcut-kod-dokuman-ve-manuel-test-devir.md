...
3. MANUEL_INCELEME / `INCELEME_GEREKLI` operasyonel kapanış akışı.
4. Merkezi şirket parametreleri.
5. Nihai bordro ön izleme.
6. Bordro kesinleştirme.
7. Gerçek üretim kullanıcı ve rol hazırlığı.
8. Canlı test veri politikası.
...
S74-C3 tam kapalıdır (`S74_C3_FULLY_COMPLETE`). Önerilen sonraki ana sprint:

- **S74-D1 — MANUEL_INCELEME karar ve çözüm akışı ön analizi / karar fazı** (`DIGER`, `INCELEME_GEREKLI` operasyonel kapanışı)

Paralel değerlendirilebilir: `PUANTAJ_OLUSTU` çakışma kullanıcı yolu; etki adayı raporlama ve dönem kapanışı bağlantısı.
