...
describe("SerbestZamanController MariaDB", () => {
  ...
  it("locks router owners, permissions and additive migration", () => {
    ...
    const migrations = readdirSync(resolve(process.cwd(), "api/migrations"))
      ...
      .sort();
    expect(migrations.at(-1)).toBe("030_haftalik_kapanis_revizyon_talepleri.sql");
  });
  ...
  it("runs HTTP serbest zaman acceptance on MariaDB", () => {
    ...
    expect(result.stdout).toContain("[PASS] NOT_PERSISTED");
    expect(result.stdout).toContain("[PASS] KARAR_BEKLIYOR NOT_ELIGIBLE");
    expect(result.stdout).toContain("[PASS] UCRET NOT_ELIGIBLE");
    expect(result.stdout).toContain("[PASS] ZERO_DAKIKA");
    expect(result.stdout).toContain("[PASS] olusum client dakika override → 422 VALIDATION_ERROR");
    expect(result.stdout).toContain("[PASS] POST iptal KULLANIM → 200");
    expect(result.stdout).toContain("[PASS] kullanim iptal bakiyeyi geri getirir");
    expect(result.stdout).toContain("[PASS] TARGET_ALREADY_CANCELLED");
    expect(result.stdout).toContain("[PASS] NO_ELIGIBLE_BALANCE");
    expect(result.stdout).toContain("[PASS] suresi_dolan_dakika includes expired olusum");
    expect(result.stdout).toContain("[PASS] donem kaydi yok POST olusum → 200");
    expect(result.stdout).toContain("[PASS] rollback: orphan event yok");
    expect(result.stdout).toContain("[PASS] rollback: orphan guard yok");
    expect(result.stdout).toContain("[PASS] rollback: partial kullanim event yok");
    expect(result.stdout).toContain("[PASS] rollback: partial duzeltme event yok");
    expect(result.stdout).toContain("[PASS] rollback: partial iptal event yok");
    expect(result.stdout).toContain("[PASS] parallel olusum → one 200 one 409");
    expect(result.stdout).toContain("[PASS] parallel olusum tek aktif guard");
    expect(result.stdout).toContain("[PASS] parallel kullanim only one success when sum exceeds");
    expect(result.stdout).toContain("[PASS] parallel kullanim negatif bakiye yok");
    expect(result.stdout).toContain("[PASS] parallel iptal → one 200 one 409");
    expect(result.stdout).toContain("[PASS] parallel iptal loser ALREADY_CANCELLED");
    expect(result.stdout).toContain("[PASS] parallel duzeltme both append 200");
    expect(result.stdout).toContain("[PASS] parallel duzeltme bakiye invariant");
    expect(result.stdout).toContain("[PASS] aciklama farkı IDEMPOTENCY_CONFLICT");
  });
});
