...
- Hastalik raporunda isveren odeme yapmaz (varsayilan).
- `ilk_iki_gun_firma_oder_mi` varsayilan `false`; kullanici rapor girisinde degistirebilir (UI: “Ilk 2 gun firma tarafindan odenecek mi?”).
- Alan yalnizca `Raporlu_Hastalik` icin; `Raporlu_Is_Kazasi` ayri degerlendirilir; rapor event/periyot kaydinda tutulur.
