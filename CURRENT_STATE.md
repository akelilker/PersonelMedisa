...
| `SGK_PERIOD_BUSINESS_DECISION` | **CLOSED_CONFIRMED** (`MG-SGK-1514-001`) | Medisa/Karyapı/Şenay Mobilya branches `1,4,5,6,7,8,9,10,11` target `AY_1_SON_GUN`; `15_TO_NEXT_MONTH_14` and `MIXED_BY_INSURED` not used |
| `SGK_PERIOD_PRODUCTION_ROLLOUT` | **OPS_ROLLOUT_ACTIVE** | released canonical readback: branch `1` = `AY_1_SON_GUN`; branches `4,5,6,7,8,9,10,11` = `NO_APPROVED_POLICY`; draft → submit → separate approval gate remains, no mutation executed |
...
- **SGK reporting period decision:** **CLOSED_CONFIRMED**; target runtime enum is `AY_1_SON_GUN`; production readback found branch `1` matched and branches `4,5,6,7,8,9,10,11` had `NO_APPROVED_POLICY`; rollout remains `OPS_ROLLOUT_ACTIVE` pending canonical apply/approval.
