...
- S75 puantaj çakışma çözümü (`/cakisma-coz`) yalnız `MUHASEBE` (`puantaj.bildirim_etki.resolve_conflict`) içindir. Migration `015` canlıdır; mevcut fixture reuse ile aday `#6` **Mevcut Puantajı Koru** (`YOK_SAYILDI`, puantaj değişmez) ve aday `#7` **Aday Etkisiyle Revize Et** (`UYGULANDI`, aynı satır UPDATE) kararları; aynı-body idempotency, farklı-karar conflict ve audit/hash/snapshot bütünlüğü canlıda kabul edilmiştir. `S75_V2` canonical projection, `S74_V1` dar legacy fallback ile geriye uyumludur. Final etiket: `S75_FULLY_COMPLETE`.
