...
describe("S98 SGK mapping + policy MariaDB acceptance", () => {
  ...
  it("covers mapping/policy dual-control and S98-R1 decision contract (incl. migration 047)", () => {
    ...
    expect(result.stdout).toContain("duplicate actor_identity_id unique rejected");
    expect(result.stdout).toContain("pending identity code");
    expect(result.stdout).toContain("revoked identity code");
    expect(result.stdout).toContain("verified actor identities work with NULL personel bridge");
